...
public:
	...
	void run() {
        ...
        // if(id == 1)
        // {
        //     if(side == 1) sprintf(command, "(init team1 (goalie))");
        //     else sprintf(command, "(init team2 (goalie))");
        //     sendMessage(command);
...
private:
	...
	void parseMsg(char * msg, const size_t len) { 
        ...
        if(id == 1) // 守门员代码
        {
            ...
            // 如果与球的角度相差太大
			if (ball_direction > 10 || ball_direction < -10) { 
				sprintf(command, "(turn %lf)", ball_direction); // 转向球的方向
				sendMessage(command);
				return;
			}
            // 当球不近也不远的时候
            if(ball_distance>3 && ball_distance<6){
                sprintf(command, "(dash 100)"); // 冲向球的位置
                sendMessage(command);
                return;
            }
            // 当球很近的时候
			if (ball_distance < 3 && isCatchBall==0) { // 如果与球的距离小于3
	...
	void messageLoop() { // 这个函数是不断接受消息并调用process，而process调用parse
		...
		while (1) {
			...
			if (ret < 0) {
				...
				// read from stdin
				if (FD_ISSET(in, &read_fds)) { // socket接收信息
